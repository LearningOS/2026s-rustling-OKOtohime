...
impl<T> LinkedList<T> {
    ...
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self where T: PartialOrd{
        ...
        if list_b.start.is_none() { return list_b; }
        ...
        let mut append_node = |node_ptr: NonNull<Node<T>>| {
            unsafe{
                match merge_end {
                    None => merge_start = Some(node_ptr),
                    Some(end_ptr) => (*end_ptr.as_ptr()).next = Some(node_ptr),
                }
                merge_end = Some(node_ptr);
            }
        };
        ...
        if let Some(remain) = head_a {
            append_node(remain);
            merge_end = list_a.end;
        }else if let Some(remain) = head_b {
            append_node(remain);
            merge_end = list_b.end;
        }
        if let Some(end_ptr) = merge_end {
            unsafe {
